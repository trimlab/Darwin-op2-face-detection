          ...
          while(true){
             ...
             try{
                 ...
                 if (detector.detect(image) > 0){
                	 ...
                	 String message = String.format("%3s", x) + ","  + String.format("%3s", y) ;
                	 out.writeBytes(message);
                     System.out.format("detect a face in %s; its center at (%s)\n", temp_filename, message);
                 }  
                 else{
                	 // send new point(-1,-1) to darwin
                	 String message = String.format("%3s", 0) + ","  + String.format("%3s", 0) ;
                	 out.writeBytes(message);
                	 System.out.format("no faces are detected in %s, default center at (%s)\n", temp_filename, message);
                 }
